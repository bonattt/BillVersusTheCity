...
[+] fix combat
    [+] Fix Sound
        ...
        [X] FIX: enemies manager is counting enemies through walls that aren't even aware as "closest" for max engaged enemies
...
PLANNED BUGFIXES
[X] FIX: existing SoundEffects should subscribe to AudioSettings, and update their volume
    STEPS TO REPRODUCE:
...
[X] FIX: when a grenade is present at the start of a level, the fuse sound plays during the dialogue. Once the dialogue finishes, the sound goes away!
...
[X] REFACTOR: to have CharCtrl movement actions (eg. sprint, crouch, roll, )
[+] Add Police Timer
    [+] Timer script
...
[X] Implement next_shot_ready sound for pump shotguns
